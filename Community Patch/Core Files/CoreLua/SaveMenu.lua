...
local allsavemodes = {Controls.NormalSaveMode, Controls.QueuedSaveMode,Controls.ForcedSaveMode,Controls.StaleSaveMode};
...
local function ButtonAbility()
	...
	if(((bUsingSteamCloud and g_SelectedEntry) or (not bUsingSteamCloud and ValidateText(text))) and CanSave()) then
		...
		Controls.SaveButton:SetToolTipString( "a" );
	...
	elseif(bUsingSteamCloud and not ValidateText(text)) then
		...
		if(not CanSave()) then
			Controls.SaveButton:SetDisabled(true);
			Controls.SaveButton:SetToolTipString( Locale.ConvertTextKey("TXT_KEY_NO_AUTOSAVE_THIS_TURN"));	
			Controls.QueueSaveButton:SetDisabled(false);
			Controls.ForceSaveButton:SetDisabled(false);			
			Controls.StaleSaveButton:SetDisabled(false);			
			Controls.QueueSaveButton:SetToolTipString( "c" );
			Controls.ForceSaveButton:SetToolTipString( "c" );	
			Controls.StaleSaveButton:SetToolTipString( "c" );	
			
		else
			Controls.SaveButton:SetDisabled(false);
			Controls.SaveButton:SetToolTipString("x");
			Controls.QueueSaveButton:SetDisabled(false);
			Controls.ForceSaveButton:SetDisabled(false);			
			Controls.StaleSaveButton:SetDisabled(false);			
			Controls.QueueSaveButton:SetToolTipString( "b" );
			Controls.ForceSaveButton:SetToolTipString( "b" );	
			Controls.StaleSaveButton:SetToolTipString( "b" );	
...
local function ShowMainButton()
	local MainButtons = {Controls.SaveButton, Controls.QueueSaveButton, Controls.ForceSaveButton, Controls.StaleSaveButton};
...
function OnSave()
	...
	Controls.NameBox:ClearString();
	SetupFileButtonList();
	if(not controlDown or GetSaveMode() == 2) then
		OnBack();
	else
		UI.ExitGame();
	end
...
function OnRequestAutoSaveButton()
	...
	AdvancedAutoSave.QueueAutoSave(5, Game.GetGameTurn() + 1);
	OnBack();
end
...
function GetDefaultSaveName()
	if (PreGame.GameStarted()) then 
		...
		local savemode = GetSaveMode();
		if(savemode == 1 or savemode == 3) then
		...
		elseif(savemode == 2) then
			...
			--return leaderDescription .. "_" .. string.format("%04d", (Game.GetLastAutoSaveTurn()));
			return leaderDescription .. "_" .. Game.GetTimeStringForYear(Game.GetLastAutoSaveTurn());
...
----------------------------------------------------------------
function OnASS()	
	UIManager:QueuePopup( Controls.AutoSaveSettingsDialog, PopupPriority.SaveMapMenu );
end
...
function InputHandler( uiMsg, wParam, lParam )
	if uiMsg == KeyEvents.KeyDown then
		if wParam == Keys.VK_ESCAPE then
			if(Controls.DeleteConfirm:IsHidden() and Controls.AutoSaveSettingsDialog:IsHidden())then
				OnBack();
			else
				if(not Controls.AutoSaveSettingsDialog:IsHidden()) then				
					Controls.AutoSaveSettingsDialog:SetHide(true);
					Controls.BGBlock:SetHide(false);
				end
...
function ShowHideHandler( isHide )
    ...
    if( not isHide ) then
    	...
    	if (PreGame.GameStarted()) then    	
    		...
    		if( PreGame.IsMultiplayerGame() or
        		...
        		Controls.RequestAutoSaveButton:SetHide( false );
        		    		...
        		    		Controls.LastAutosaveTypeLabel:SetText("Last AutoSave Type: " .. "???");
    	
				    	if(Game.GetLastAutoSaveTurn() >= 0) then
				    		if(Game.GetLastAutoSaveTurn() == Game.GetGameTurn()) then
				    			Controls.LastAutosaveTurnLabel:SetText("Last AutoSave Turn: " .. Game.GetLastAutoSaveTurn() .. "  (current turn)");
				    		elseif(Game.GetLastAutoSaveTurn() == Game.GetGameTurn() - 1) then
				    			Controls.LastAutosaveTurnLabel:SetText("Last AutoSave Turn: " .. Game.GetLastAutoSaveTurn() .. "  (1 turn ago)");
				    		else
				    			Controls.LastAutosaveTurnLabel:SetText("Last AutoSave Turn: " .. Game.GetLastAutoSaveTurn() .. "  (" ..(Game.GetGameTurn() - Game.GetLastAutoSaveTurn()) .. " turns ago)");
				    		end
				    	else 
				    	...
				    	Controls.RequestAutoSaveButton:SetHide(not AdvancedAutoSave.GetData().AllowAutoSaveRequests);
				    	Controls.AutoSaveDetails:SetHide(not AdvancedAutoSave.GetData().ShowLastAutoSaveDetails);
				    	...
				    	Controls.ForceSaveButtonLabel:SetText("[ICON_FLOWER][ICON_INVEST][COLOR_FONT_RED]Force [COLOR_WHITE]" .. Locale.ConvertTextKey("TXT_KEY_MENU_SAVE"));
				    	Controls.QueueSaveButtonLabel:SetText("[COLOR_FONT_GREEN]Queue [COLOR_WHITE]" .. Locale.ConvertTextKey("TXT_KEY_MENU_SAVE"));
				    	Controls.StaleSaveButtonLabel:SetText("[COLOR_MAGENTA]Stale [COLOR_WHITE]" .. Locale.ConvertTextKey("TXT_KEY_MENU_SAVE"));
      ...
      Controls.NameBox:TakeFocus();
			...
			OnSaveModeChanged()
			ButtonAbility();
			if(AdvancedAutoSave.IsActive()) then
