	...
	else
	{
		...
		if(hUnit->canMove())
		{
			...
			else if (pkQueueData->eMissionType == CvTypes::getMISSION_ESTABLISH_TRADE_ROUTE())
			{
				...
				if (pPlot)
				{
					NET_MESSAGE_DEBUG_OSTR_ALWAYS(__FUNCTION__ " trade route mission");
					// This should fix cargo related desyncs (different paths etc). I suspect there is still the potential for desyncs if the user opens the TR popup but does not issue the route command right away. I think a net message is in order for a real fix.
